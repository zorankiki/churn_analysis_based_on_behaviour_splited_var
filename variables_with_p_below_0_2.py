...
def get_one_export(data_dir, date_dir, exports_dir_path, filename, model_type, model_number, model_name):
    ...
    coef_and_pvalues = \
    bucket.load_csv_from_s3(file_name = data_dir+'data/'+date_dir+'/'+exports_dir_path+'/'+model_type+'/'+\
    'model_'+str(model_number)+'/'+filename)
...
def get_significant_variables(data_dir, date_dir, exports_dir_path_from, exports_dir_path_to, model_type, model_number, p_limit, sort_by = 'variable_name/p_value'):
    ...
    ### leave only variables whose p < 0.2 ###
    export = export[export['p value'].apply(lambda x: np.any(np.array(x)<0.2), axis = 1)]
    ...
    if sort_by == 'p_value':
        ...
        file_name = 'variables_with_p_below_0_2_sorted_by_p_value.csv', \
        dir = '/'+data_dir+'data/'+date_dir+'/'+exports_dir_path_to+'/'+model_type+'/'+'model_'+str(model_number)+'/')

        export.to_csv('data/'+date_dir+'/'+exports_dir_path_to+'/'+model_type+'/'+'model_'+str(model_number)+'/variables_with_p_below_0_2_sorted_by_p_value.csv', index = False)


    elif sort_by == 'variable_name':
        bucket.store_csv_to_s3(data_frame = export, \
        file_name = 'variables_with_p_below_0_2_sorted_by_variable_name.csv', \
        dir = '/'+data_dir+'data/'+date_dir+'/'+exports_dir_path_to+'/'+model_type+'/'+'model_'+str(model_number)+'/')

        export.to_csv('data/'+date_dir+'/'+exports_dir_path_to+'/'+model_type+'/'+'model_'+str(model_number)+'/variables_with_p_below_0_2_sorted_by_variable_name.csv', index = False)
