...
def calculate_churn_risk(date_of_analysis, df_timeline, vars_, data_all_spots, ctv, model_type, model_number, prediction_month, with_add_vars, p_limit = 0.5, event_date_type='cancellation_requested/cancellation_confirmed', spots_set='ALL/CAN_CANCEL', with_wo_CB='with_CB/wo_CB', with_dontKnow_non_SH_site_spots = True, with_spot_categories_metro_areas=True):
    ...
    churn_risk_prediction_testing_path = churn_risk_prediction_path + 'testing/'
    churn_risk_prediction_with_add_important_vars_testing_path = churn_risk_prediction_with_add_important_vars_path + 'testing/'
    ...
    data_all_spots.drop(data_all_spots[data_all_spots['right_limit']>prediction_month].index, inplace = True)
    ...
    data_all_spots.loc[(data_all_spots['right_limit']==prediction_month)&\
                      (data_all_spots['event']==True), 'event'] = False
    ...
    if event_date_type == 'cancellation_confirmed':
        file_name = 'churn_risk_based_on_'+spots_set+'_spots_'+with_wo_CB+'_cancellation_confirmed_p_below_'+p_limit+'_'+prediction_month_+'.csv'
    elif event_date_type == 'cancellation_requested':
        file_name = 'churn_risk_based_on_'+spots_set+'_spots_'+with_wo_CB+'_cancellation_requested_p_below_'+p_limit+'_'+prediction_month_+'.csv'
    ...
    if with_add_vars == False:
        if not os.path.exists(churn_risk_prediction_testing_path):
            os.makedirs(churn_risk_prediction_testing_path)
        churn_risk_sorted_spots.to_csv(churn_risk_prediction_testing_path+file_name, index = False)
        bucket.store_csv_to_s3(data_frame = churn_risk_sorted_spots, \
        file_name = file_name, \
        dir = '/'+churn_based_on_behaviour_dir+churn_risk_prediction_testing_path)
    else:
        if not os.path.exists(churn_risk_prediction_with_add_important_vars_testing_path):
            os.makedirs(churn_risk_prediction_with_add_important_vars_testing_path)
        churn_risk_sorted_spots.to_csv(churn_risk_prediction_with_add_important_vars_testing_path+file_name, index = False)
        bucket.store_csv_to_s3(data_frame = churn_risk_sorted_spots, \
        file_name = file_name, \
        dir = '/'+churn_based_on_behaviour_dir+churn_risk_prediction_with_add_important_vars_testing_path)
