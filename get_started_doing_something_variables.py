...
def main(date_of_analysis, variables_to_use_for_the_model = []):
    ...
    ## did something before and didnt last month, did something last month ##
    did_something_last_month_vars = ['had_'+x[0]+'_'+x[1] for x in period_to_look_at_stopped]
    did_something_before_and_didnt_last_month_vars = ['had_'+x[0]+'_before_and_didnt_'+x[1] for x in period_to_look_at_stopped]
    ...
    for key in all_vars_base_names.keys():
        if 'changed' in key.lower() and 'inquiry' in key.lower():
            ...
            did_something_before_and_didnt_last_month_vars.append('had_'+for_properly_used_inquiries_vars['changed_inquiry_status_to_properly_used'][all_vars_base_names[key]]+'_before_and_didnt_'+period_to_look_at_stopped_tmp)
            did_something_before_and_didnt_last_month_vars.remove('had_'+all_vars_base_names[key]+'_before_and_didnt_'+period_to_look_at_stopped_tmp)



    return (variables_to_use_for_the_model, did_something_last_month_vars, did_something_before_and_didnt_last_month_vars)
